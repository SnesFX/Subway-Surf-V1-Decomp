...
public class UIPopupList : MonoBehaviour
{
	...
	private void Highlight(UILabel lbl, bool instant)
	{
		...
		if (!(component != null) || !component.enabled)
		{
			...
			Vector3 vector = lbl.cachedTransform.localPosition + new Vector3(0f - num, y, 0f);
	...
	private void OnClick()
	{
		if (mChild == null && atlas != null && font != null && items.Count > 1)
		{
			...
			for (int count = items.Count; i < count; i++)
			{
				...
				uILabel.cachedTransform.localPosition = new Vector3(border.x, num3, 0f);
			...
			a = Mathf.Max(a, bounds.size.x - border.x * 2f);
			...
			mHighlight.cachedTransform.localScale = new Vector3(a - border.x * 2f + (sprite.inner.xMin - sprite.outer.xMin) * 2f, num2 + num * 2f, 1f);
