...
public class UIPanel : MonoBehaviour
{
	...
	private int GetChangeFlag(UINode start)
	{
		...
		if (num == -1)
		{
			...
			int i = 0;
			for (int count = mHierarchy.Count; i < count; i++)
			{
				UINode uINode2 = mHierarchy[i];
	...
	private void UpdateTransforms()
	{
		...
		if (!widgetsAreStatic || flag2 != mCulled)
		{
			int i = 0;
	...
	private void LateUpdate()
	{
		...
		int j = 0;
		for (int count2 = mChanged.Count; j < count2; j++)
		{
			Fill(mChanged[j]);
		}
		UpdateDrawcalls();
